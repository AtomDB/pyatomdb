  ...
  def set_response(self, rmf, arf=False, raw=False, sparse=False):
    ...
    # Use chanoffset to correct for this.
      ...
      if not sparse:
        ...
        nmatrixelements = 0
        for i in range(len(self.rmf[matrixname].data)):
          nmatrixelements+=numpy.int32(sum(self.rmf[matrixname].data[i]['N_CHAN']))
        
        
        # need to specifyy data types, particularly integers. Numbers get big...
        # 3 1-d arrays to store the matrix elements. data is the actual matrix
        # row is the final element (i.e. energy bin the photon is recorded in)
        # col is the initial element (actual energy bin of incoming photon)
        
        data = numpy.zeros(nmatrixelements, dtype=numpy.float32)
        row = numpy.zeros(nmatrixelements, dtype=numpy.int32)
        col = numpy.zeros(nmatrixelements, dtype=numpy.int32)
        
        inextdatabin=numpy.int32(0)
        
        # now get assigning
        for ibin, i in enumerate(self.rmf[matrixname].data):
          lobound = 0
          for igrp in range(i['N_GRP']):
            fchan = i['F_CHAN'][igrp]
            nchan = i['N_CHAN'][igrp]
            
            #Correct for the zero or one starting index
            fchan -= chanoffset

            data[inextdatabin:inextdatabin+nchan] = i['MATRIX'][lobound:lobound+nchan]
            row[inextdatabin:inextdatabin+nchan] = fchan+numpy.arange(nchan, dtype=numpy.int32)
            col[inextdatabin:inextdatabin+nchan] = ibin
...
#        t0 = time.time()
      ...
      # read the ionization balance table
        ...
        #print(energy_kev)
          ...
          u_th_sq= 2*const.BOLTZMAN_CONSTANT*T/(llist['Element'][0]*(const.PROTON_MASS)*((100*(const.LIGHTSPEED))**2))
          u_turb_sq =  2* (((1000*velocity_broadening)/const.LIGHTSPEED)**2)
          delta_E = 1.60218e-9*energy_kev*math.sqrt(u_th_sq + u_turb_sq )
          ...
          if numpy.array(Ab).size==30:
           ...
           # tau = N_e*0.83*Abund[llist['Element'][0]-1]*ion*(math.pi**0.5)*(2**0.5)*Ab[llist['Element'][0]-1]*(const.PLANCK_CONSTANT)*(const.CLASSICAL_ELECTRON_RADIUS)*(const.LIGHTSPEED*100)*llist['Oscil_str']/delta_E
            ...
            ## now estimating tau as t(E)
            #nrgbins = numpy.linspace(numpy.min(eedges), numpy.max(eedges), 60000)
            nrgbins = eedges
            total_tau_x = numpy.zeros(len(nrgbins))

            
            for nrg in range(len(energy_kev)):
              #natural_width = ((llist['EINSTEIN_A'][nrg]) * 6.582e-16)/1000 ## delE = hbar*A
              natural_width = 1e-4 ## putting a small value for now

              doppler_broad = energy_kev[nrg]*(wcoeff)
              #doppler_broad = delta_E_kev[nrg]

              #print('***',energy_kev[nrg], natural_width, doppler_broad[0])

              voigt_cdf = Voigt1D(energy_kev[nrg], 1.0, natural_width, doppler_broad[0])
              
              y_cdf = voigt_cdf(nrgbins)
              y_cdf /= numpy.max(y_cdf)  # normalize to peak = 1

              if nrg == 0:
                total_tau_x = y_cdf * tau[nrg]
              else:
                total_tau_x += y_cdf * tau[nrg]

            ## interpolating to use tau_x as a function of E
            #tau_x = UnivariateSpline(nrgbins, total_tau_x, s=0)
            tau_x = total_tau_x
            
            
            ##for nrg in range(len(energy_kev)):
            ##  print(energy_kev[nrg],tau[nrg], tau_x(energy_kev[nrg]))



              #tau_x = tau[nrg]*voigt_cdf(nrg_bins)

              ##print(energy_kev[nrg], tau[nrg])
        ...
        factor1 = numpy.zeros_like(eedges)
        for iline in igood:
          ...
          elif tau_line > 0.1 and tau_line<10:

            ## estimate factor based on tau value
            #factor1 = numpy.zeros(len(eedges))
            #for ee in range(len(eedges)):

            mask = (eedges > energy_kev[iline]-(width[iline])) & (eedges <= energy_kev[iline]+(width[iline]))
            eedges_filtered = eedges[mask]
            factor1 = numpy.zeros_like(eedges)

            factor = (0.00001*(tau_x(eedges_filtered)**4))-(0.0008*(tau_x(eedges_filtered)**3))+(0.0209*(tau_x(eedges_filtered)**2))-(0.2254*tau_x(eedges_filtered))+0.9828
            #factor = ((1-(math.exp(-tau[iline])))/(tau[iline]))
            #factor1[mask] = 1 - factor
            #factor1[mask] = -0.22 * numpy.exp(-(eedges_filtered - energy_kev[iline])**2 / (2 * 0.002**2))
            
            #print('***',energy_kev[iline], width[iline], tau_x(eedges_filtered), factor1[mask], eedges_filtered)    


            spec += broaden_object.broaden(energy_kev[iline],width[iline],eedges)*llist['Epsilon'][iline]

            #print(eedges_filtered, (broaden_object.broaden(const.HC_IN_KEV_A/llist['Lambda'][iline],width[iline],eedges)*llist['Epsilon'][iline])[mask], factor1[mask])
            #print('***',energy_kev[iline])

            #print(broaden_object.broaden(const.HC_IN_KEV_A/llist['Lambda'][iline],width[iline],eedges)*llist['Epsilon'][iline], (broaden_object.broaden(const.HC_IN_KEV_A/llist['Lambda'][iline], delta_E_kev[iline],eedges)*llist['Epsilon'][iline] *factor1)) 


          
          elif  tau_line>=10 and tau_line<23:

            factor =(-0.000004*(tau_line**3))+(0.0005*(tau_line**2))-(0.0189*tau_line)+0.2468
            #factor = ((1-(math.exp(-tau[iline])))/(tau[iline]))
            factor1 = 1-factor
...
#        print("Broadeninging %i lines, in %f seconds"%(len(igood), t1-t0))
          ...
          tau_line = tau[iline]
          mask = (eedges > const.HC_IN_KEV_A/llist['Lambda'][iline]-(width[iline])) & (eedges <= const.HC_IN_KEV_A/llist['Lambda'][iline]+(width[iline]))
          eedges_filtered = eedges[mask]
          tau_filtered = tau_x[mask]
          #if  tau_line < 0.1:
          #  factor1[mask] = 1.0

          if tau_line >= 0.5:

            #factor = 1 - ((0.00001*(tau_x(eedges_filtered)**4))-(0.0008*(tau_x(eedges_filtered)**3))+(0.0209*(tau_x(eedges_filtered)**2))-(0.2254*tau_x(eedges_filtered))+0.9828)
            #factor = 1 - ((0.00001*(tau_filtered**4))-(0.0008*(tau_filtered**3))+(0.0209*(tau_filtered**2))-(0.2254*tau_filtered)+0.9828)
            factor = 1 - (0.936*numpy.exp(-0.276*tau_filtered)+0.040)
            factor1[mask] = -factor
            #factor1[mask] = -0.24 * numpy.exp(-(eedges_filtered - const.HC_IN_KEV_A/llist['Lambda'][iline])**2 / (2 * 0.00157**2))
            #print('***', const.HC_IN_KEV_A/llist['Lambda'][iline],width[iline], eedges_filtered)
            #voigt_cdf = Voigt1D(const.HC_IN_KEV_A/llist['Lambda'][iline], 1.0, 1e-4, width[iline])
              
            #y_cdf = voigt_cdf(eedges)
            #y_cdf /= numpy.max(y_cdf)  # normalize to peak = 1
            #factor1 += -0.30 * y_cdf
        spec = spec[1:]-spec[:-1]
        factor1 = (0.82*factor1[1:]+0.18*factor1[:-1])
        #factor1 = factor1[1:]
        spec = spec + (spec*factor1)
        #mask = (eedges[:-1] > 6.69) & (eedges[:-1] <= 6.71)
        #print('***', eedges[:-1][mask], spec[mask], factor1[mask])
